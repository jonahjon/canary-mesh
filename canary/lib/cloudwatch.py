...
class Cloudwatch(object):
    def __init__(self, logger, **kwargs):
        ...
        if kwargs is not None:
            if kwargs.get('credentials') is None:
                ...
                self.cw_client = boto3.client('cloudwatch', region_name=kwargs.get('region'))
            else:
    ...
    def put_metric_alarm(self, AlarmName, AlarmActions):
        method = inspect.stack()[0][3]
        self.logger.info('Executing function {}'.format(method))
        try:
            response = self.cw_client.modify_spot_fleet_request(
                AlarmName=AlarmName,
                AlarmActions = [
                    AlarmActions
                ]
